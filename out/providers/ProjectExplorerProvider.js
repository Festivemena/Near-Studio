...
class ProjectExplorerProvider {
    ...
    async _parseContractABI(accountId) {
        try {
            ...
            if (abiContent && abiPath) {
                ...
                const action = await vscode.window.showInformationMessage(`Contract deployed successfully${accountId ? ` to ${accountId}` : ''}! Contract ABI parsed.`, 'Open Contract Interface', 'View ABI');
    ...
    async _openContractInterface(abiContent, accountId) {
        // Create a new webview panel for contract interaction
        const panel = vscode.window.createWebviewPanel('nearContractInterface', `NEAR Contract Interface${accountId ? ` - ${accountId}` : ''}`, vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async _callContractMethod(method, args, accountId) {
    ...
    async _viewContractMethod(method, args, accountId) {
    ...
    _getContractInterfaceHtml(abiContent, accountId) {
